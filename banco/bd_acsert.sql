create database `bd_acsert` DEFAULT CHARSET=utf8;

use `bd_acsert`;

create table if not exists `p_fisica`(
`id_p_fisica` int auto_increment primary key,
...
`formacao_acad` varchar(60) not null,
`uf` varchar(30) not null,
`municipio` varchar(30) not null,
`cep` varchar(11) not null,
`endereco` varchar(30) not null,
`n_logradouro` int not null,
`complemento`varchar(30) null,
`telefone` varchar(20) not null,
`celular` varchar(20) not null,
`email` varchar(30) not null,
`site` varchar(30) null,
`rec_novidades` varchar(5) not null default 'não'
);
...
`n_funcion` varchar(18) not null,
`p_contato` varchar(60) not null,
...
`hora` time not null,
`forma_contato` varchar(20) not null,
`forma_conhec` varchar(50) not null,
constraint `FK_P_FISICA`  foreign key (`id_p_fisica`) references `p_fisica`(`id_p_fisica`) on update cascade on delete cascade,
constraint `FK_P_JURIDICA`  foreign key (`id_p_juridica`) references `p_juridica`(`id_p_juridica`) on update cascade on delete cascade
);
